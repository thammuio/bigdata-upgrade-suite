...
-- calculate the sum over all rows from the top row till this row
from Sales_Data
...
-- calculate the sum over all rows from the top row till this row - another way
from Sales_Data
select OrderID,storeID,ProductID,OrderDate,Revenue,
sum(Revenue) over(order by OrderID) as Running_Total;

-- calculate the sum over all rows from the top row till this row - for a date
from Sales_Data
select  OrderID,storeID,ProductID,OrderDate,Revenue,
sum(Revenue) over(partition by OrderDate order by OrderID) 
as Running_Total;

-- MOVING AVERAGE OF THE REVENUE
from Sales_Data
select  OrderID,storeID,ProductID,OrderDate,Revenue,
